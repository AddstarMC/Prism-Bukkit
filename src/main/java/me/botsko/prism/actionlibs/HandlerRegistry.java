...
public class HandlerRegistry<H> {
	...
	public void registerCustomHandler(Plugin apiPlugin, Class<? extends Handler> handlerClass) throws InvalidActionException {
		// Is plugin allowed?
		@SuppressWarnings("unchecked") ArrayList<String> allowedPlugins = (ArrayList<String>) Prism.config.getList("prism.tracking.api.allowed-plugins");
	...
	public Handler getHandler(String name) {
		if (name != null) {
			if (registeredHandlers.containsKey(name)) {
				try {
					Class<? extends Handler> handlerClass = registeredHandlers.get(name);
